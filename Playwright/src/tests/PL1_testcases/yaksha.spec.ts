...
test.describe("Yaksha", () => {
  ...
  */
  ...
  test("1_Verify the Logout functionality", async ({ page }) => {
    await verifyLogout(page, BaseUrl);
   
    await loginPage.performLogOut(BaseUrl);
    await verifyLogoutt(page, BaseUrl);
  });
  ...
  test("TS-2 Verify New holidays could be created ", async ({ page }) => {
    ...
    await verifyCreateNewLeaveRequest(page);
  });
  ...
  test("TS-3 Verify an error appears when password and confirm password mismatch", async ({
    ...
    await verifyConfirmPassword(page);
 ...
 */
  ...
  test("TS-6 Verify Image could be Uploaded as profile Pic", async ({
    ...
    await verifyUploadProfilePic(page);
  });
 ...
 */
  ...
  test("TS-10 Verify tooltip shows up when hovered over 'Help' button (Question mark)", async ({
    ...
    await verifyHelpHover(page, BaseUrl);
  });
});
...
//verify functions for assertions of testcase 1-1;
async function verifyLogout(page: Page, BaseUrl: string) {
...
//verify functions for assertions of testcase 1-2;
async function verifyLogoutt(page: Page, BaseUrl: string) {
  await assertUrl(page.url(),
...
//verify functions for assertions of testcase 2;
async function verifyCreateNewLeaveRequest(page: Page) {
...
//verify functions for assertions of testcase 3;
async function verifyConfirmPassword(page: Page) {
...
//verify functions for assertions of testcase 4;
async function verifyAdminAdd(page: Page) {
...
// helper functions for assertions of testcase 5;
async function vefityGenInfoTab(page: Page) {
  const Message = await page.locator("//h6[text()='General Information']").textContent();
    expect(Message).toBe("General Information");
}

// helper functions for assertions of testcase 6;
async function verifyUploadProfilePic(page: Page) {
...
//helper functions for assertions of testcase 7;
async function vefityOrgStructure(page: Page) {
...
// helper functions for assertions of testcase 8;
async function verifyAdminSort(page: Page) {
  await page.waitForTimeout(1000);
  const sortingButton = page.locator('i.oxd-table-header-sort-icon').nth(0);
  await expect(sortingButton).toBeVisible();
}

// helper functions for assertions of testcase 10;
async function verifyHelpHover(page: Page, BaseUrl: string) {
  expect(page.url()).toBe(BaseUrl + "/web/index.php/admin/viewSystemUsers");
  await page.waitForTimeout(2000);
...
async function assertSortedListDescending(
  ...
  for (let i = 0; i < actualList.length - 1; i++) {
    if (actualList[i].localeCompare(actualList[i + 1]) < 0) {
      throw new Error(
        `List is not sorted in descending order at index ${i}: '${actualList[i]
        }' < '${actualList[i + 1]}'`
      );
    }
  }
